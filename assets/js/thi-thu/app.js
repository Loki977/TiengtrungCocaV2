...
function renderQuestion() {
  ...
  els.questionContext.innerHTML = question.context
    ? `<span class="hanzi">${escapeHtml(question.context)}</span>${question.contextPinyin ? `<span class="pinyin">${escapeHtml(question.contextPinyin)}</span>` : ''}`
    : '';
  els.questionContext.hidden = !question.context;
  const showQuestionText = question.sectionId === 'reading' && Boolean(question.hanzi);
  els.questionText.innerHTML = showQuestionText
    ? `<span class="hanzi">${escapeHtml(question.hanzi)}</span>${question.pinyin ? `<span class="pinyin">${escapeHtml(question.pinyin)}</span>` : ''}`
    : '';
  els.questionText.hidden = !showQuestionText;
  ...
  if (question.type === 'single_choice') {
    for (const option of question.options) {
      ...
      label.innerHTML = `
        ...
        <span class="option-copy"><span><strong>${escapeHtml(option.id)}.</strong> ${escapeHtml(option.text)}</span>${option.pinyin ? `<small class="pinyin">${escapeHtml(option.pinyin)}</small>` : ''}</span>
      `;
...
function renderReview() {
  ...
  state.lastResult.details.forEach((item, index) => {
    ...
    article.innerHTML = `
      ...
      ${item.question.transcript ? `<p><strong>Nội dung audio:</strong> ${escapeHtml(item.question.transcript)}</p>` : ''}
